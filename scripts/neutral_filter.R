...
### SEQUENCES LENGTH CHECK
if (opt$length_check == "yes") {
	N <- length(input_files)
	Results <- data.frame(id=input_files, dif_length=logical(N), non3_mult=logical(N))
	for (i in 1:N) {
		myseqs <- readDNAStringSet(file=paste(input_dir, input_files[i], sep="/"), format="fasta")
		lengths <- width(myseqs)
		if (length(unique(lengths)) > 1) {
			Results$dif_length[i] <- T
		}
		if (all.equal(lengths/3, as.integer(lengths/3))!=T) {
			Results$non3_mult[i] <- T
		}
	}
	pos <- which(Results$dif_length==T | Results$non3_mult==T)
	if (length(pos) > 0) {
		print("Not all files contain sequences of equal and multiple of 3 length. Fix this before filtering neutral sites")
		problems <- list(problematic_files=Results[pos,])
		print(problems)
		print(paste0("Sequences Length Check progress ", round(i*100/N, 4), "%"))
	} else {
		print("Ok! All files contain sequences of equal and multiple of 3 length")
	}
	print("Now can filter neutral sites by choosing 'no' for --length_check and --non_gact")
	if (opt$non_gact == "no") {
		quit()
	} else {
		rm(Results, problems, myseqs, lengths, pos)
	}
}


### UNKNOWN CODONS REMOVAL
...
if (opt$non_gact == "yes") {
	...
	for (i in 1:N) {
		...
		for (j in 1:M) {
			...
			pos <- unique(c(pos, which(mystring!="g" & mystring!="G" & mystring!="a" & mystring!="A" & mystring!="c" & mystring!="C" & mystring!="t" & mystring!="T")))
		}
		cod <- unique(floor(pos/3-0.000000000001) + 1)
		pos2 <- as.integer(c(cod*3-2, cod*3-1, cod*3))
		if (length(pos2)>0) {
			for (j in 1:M) {
				mystring <- s2c(myseqs[[j]])
				mystring <- mystring[-pos2]
				myseqs[[j]] <- c2s(mystring)
			}
			print(paste0(input_files[i], ": Not all codons were retained for this gene"))
		}
		write.fasta(myseqs, names=names(myseqs), as.string=TRUE, file.out=paste(output_dir, input_files[i], sep="/"))
		print(paste0("Unknown Codons Removal progress ", round(i*100/N, 4), "%"))
	}
	print("Now can filter neutral sites by choosing 'no' for --length_check and --non_gact")
	quit()
}
