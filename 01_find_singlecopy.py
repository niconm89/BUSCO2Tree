	...
	for busco_id in common_busco_ids:
		...
		with open(busco_multifasta, 'wt') as MULTIFASTA:
			for genome in genomes_names:
				busco_fasta_file = os.path.join(BUSCODIR, genome, "run_" + LINEAGE + "_" + ODB, "busco_sequences/single_copy_busco_sequences", busco_id + '.faa')
				#CHECK FILE EXISTS
				with open(busco_fasta_file, 'rt') as FASTA:
					for line in FASTA:
						if line[0] == '>':
							MULTIFASTA.write(">" + genome + "_" + line[1:])
						else:
